...
public class TrackingCbrRecordStore extends DelegatingCbrRecordStore {

    private static final Logger LOG = Logger.getLogger(TrackingCbrRecordStore.class);

    private final CbrRetrievalTracker tracker;
    private final Consumer<CbrRetrievalRecorded> eventSink;

    @Inject
    TrackingCbrRecordStore(@Delegate @Any CbrRecordStore delegate,
                           CbrRetrievalTracker tracker,
                           Event<CbrRetrievalRecorded> recordedEvent) {
        this(delegate, tracker, recordedEvent::fire);
    }

    TrackingCbrRecordStore(CbrRecordStore delegate,
                           CbrRetrievalTracker tracker,
    ...
    public <C extends CbrRecord> List<CbrMatch<C>> retrieveSimilar(
        ...
        try {
            String traceId = tracker.record(query, (List<CbrMatch<?>>) (List<?>) results);
            var traced = results.stream()
                    .map(s -> new CbrRetrievalTrace.TracedCase(
                            ...
                            s.cbrRecord().trustScore(), s.cbrRecord().producerAgentId(),
