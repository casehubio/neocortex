...
class TrackingCbrRecordStoreTest {
    ...
    @Test void recordsTraceAndFiresEvent() {
        ...
        var c = new CbrFeatureRecord("p", "s", null, Confidence.unknown(0.9), Map.of(), null, null);
        var results = List.<CbrMatch<CbrFeatureRecord>>of(
                new CbrMatch<>(c, "c1", "test-type", 0.85));
        var delegate = stubDelegate(results);
        var decorator = new TrackingCbrRecordStore(delegate, tracker, eventRef::set);

        var query = CbrQuery.of("t1", new MemoryDomain("cbr"), io.casehub.platform.api.path.Path.root(), "default", Map.of(), 5);
        var returned = decorator.retrieveSimilar(query, CbrFeatureRecord.class);
    ...
    @Test void trackerFailure_nonFatal() {
        CbrRetrievalTracker failingTracker = new CbrRetrievalTracker() {
            @Override public String record(CbrQuery q, List<CbrMatch<?>> r) {
        ...
        var results = List.<CbrMatch<CbrFeatureRecord>>of(
                new CbrMatch<>(c, "c1", "test-type", 0.85));
        var delegate = stubDelegate(results);
        var decorator = new TrackingCbrRecordStore(delegate, failingTracker, e -> {});
    ...
    @Test void resultsUnchanged() {
        ...
        var c = new CbrFeatureRecord("p", "s", null, Confidence.unknown(0.9), Map.of(), null, null);
        var results = List.<CbrMatch<CbrFeatureRecord>>of(
                new CbrMatch<>(c, "c1", "test-type", 0.85, true, Map.of("f", 0.9), null, io.casehub.platform.api.path.Path.root(), null));
        var delegate = stubDelegate(results);
        var decorator = new TrackingCbrRecordStore(delegate, tracker, e -> {});
    ...
    void trustTrajectory_convertedToLabel() {
        var tracker  = new InMemoryCbrRetrievalTracker();
        var eventRef = new AtomicReference<CbrRetrievalRecorded>();
        var c        = new CbrFeatureRecord("p", "s", null, Confidence.unknown(0.9), Map.of(), 0.8, "agent-1");
        var scored = new CbrMatch<>(c, "c1", "test-type", 0.85, false, Map.of(), null,
    ...
    void trustTrajectory_improving() {
        var tracker  = new InMemoryCbrRetrievalTracker();
        var eventRef = new AtomicReference<CbrRetrievalRecorded>();
        var c        = new CbrFeatureRecord("p", "s", null, Confidence.unknown(0.9), Map.of(), 0.5, "agent-1");
        var scored = new CbrMatch<>(c, "c1", "test-type", 0.85, false, Map.of(), null,
    ...
    private CbrRecordStore stubDelegate(List<? extends CbrMatch<?>> results) {
