package io.casehub.memory.cbr.qdrant;
...
public class QdrantCbrCaseMemoryStore implements CbrCaseMemoryStore {
    ...
    private MemoryInput serializeToMemoryInput(CbrCase cbrCase, String entityId,
        ...
        // Discriminator: textual or feature-vector
        if (cbrCase instanceof FeatureVectorCbrCase fv) {
            attributes.put("cbr.type", "feature-vector");
            // Serialize features to JSON
            try {
                String featuresJson = MAPPER.writeValueAsString(fv.features());
                attributes.put("cbr.features", featuresJson);
            } catch (JsonProcessingException e) {
                throw new RuntimeException("Failed to serialize features to JSON", e);
            }
        } else {
            attributes.put("cbr.type", "textual");
        }
    ...
    private <C extends CbrCase> C reconstructCase(Map<String, Value> payload, Class<C> caseClass) {
        ...
        if (featuresJson != null && (caseClass == FeatureVectorCbrCase.class
                || caseClass == CbrCase.class || caseClass.isAssignableFrom(FeatureVectorCbrCase.class))) {
            try {
                Map<String, Object> features = MAPPER.readValue(featuresJson, MAP_TYPE);
                return (C) new FeatureVectorCbrCase(problem, solution, outcome, confidence, features);
            } catch (Exception e) {
                LOG.log(Level.WARNING, "Failed to deserialize features JSON", e);
            }
        }

        if (caseClass == TextualCbrCase.class || caseClass == CbrCase.class
                || caseClass.isAssignableFrom(TextualCbrCase.class)) {
            return (C) new TextualCbrCase(problem, solution, outcome, confidence);
        }
