...
import io.casehub.memory.cbr.CbrCase;
import io.casehub.memory.cbr.FeatureVectorCbrCase;
...
final class CbrPointBuilder {
    ...
    static PointStruct buildPoint(CbrCase cbrCase, String caseType,
        ...
        // Feature fields: store individually as payload for filtering,
        // and as JSON blob for reconstruction
        if (cbrCase instanceof FeatureVectorCbrCase fv && !fv.features().isEmpty()) {
            ...
            // Store full features as JSON for deserialization
            try {
        ...
        // Store full case class name for deserialization
        payload.put("_case_class", ValueFactory.value(cbrCase.getClass().getName()));
