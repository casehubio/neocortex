...
@Priority(2)
@ApplicationScoped
public class InMemoryCbrRecordStore implements CbrRecordStore {
    ...
    public String store(CbrRecord cbrRecord, String caseType, String entityId, MemoryDomain domain,
        ...
        cases.add(new StoredCase(id, cbrRecord, caseType, entityId, domain, tenantId, caseId, Instant.now(), null, null, null, null, scope, null));
        return id;
    }

    @Override
    public String store(CbrRecord cbrRecord, String caseType, io.casehub.neocortex.memory.Subject subject, MemoryDomain domain,
                        String tenantId, String caseId, io.casehub.platform.api.path.Path scope,
    ...
    public <C extends CbrRecord> List<CbrMatch<C>> retrieveSimilar(CbrQuery query, Class<C> caseClass) {
        ...
        for (StoredCase stored : cases) {
            ...
            if (!caseClass.isInstance(stored.cbrRecord())) {continue;}
            ...
            if (!query.filters().isEmpty()) {
                ...
                if (!matchesFilters(stored.cbrRecord(), query.filters(), candidateSchema)) {continue;}
            }
            ...
            if (!dtwBandFields.isEmpty() && candidates.size() >= query.topK()) {
                ...
                if (kthScore > 0) {
                    ...
                    for (FeatureField.TimeSeries ts : dtwBandFields) {
                        ...
                        FeatureValue caseTs     = stored.cbrRecord().features().get(ts.name());
            ...
            CbrSimilarityScorer.SimilarityBreakdown breakdown = CbrSimilarityScorer.scoreDetailed(
                    query.features(), stored.cbrRecord().features(), query.weights(), candidateSchema, Map.of(),
                    abandonCost);

            double score = breakdown.score();
            if (score >= query.minSimilarity()) {
                candidates.add(new CbrMatch<>((C) stored.cbrRecord(), stored.caseId(), stored.caseType(),
    ...
    public void recordOutcome(String caseId, String tenantId, CbrOutcome outcome) {
        for (int i = 0; i < cases.size(); i++) {
            ...
            if (caseId.equals(stored.caseId()) && tenantId.equals(stored.tenantId())) {
                ...
                CbrRecordSchema schema = schemas.get(stored.caseType());
                ...
                io.casehub.neocortex.cognitive.Confidence newConfidence = CbrOutcome.adjustConfidence(
                        stored.cbrRecord().confidence(), outcome.successRate(), lr);
                CbrRecord updated = stored.cbrRecord().withOutcome(
    ...
    public Integer purge(CbrRetentionPolicy policy) {
        ...
        if (policy.minTrustScore() != null) {
            cases.removeIf(sc -> sc.tenantId().equals(policy.tenantId())
                                 && sc.domain().equals(policy.domain())
                                 && (policy.caseType() == null || sc.caseType().equals(policy.caseType()))
                                 && sc.cbrRecord().trustScore() != null
                                 && sc.cbrRecord().trustScore() < policy.minTrustScore());
        }
        return before - cases.size();}
    ...
    public CbrScanResult scan(CbrScanRequest request) {
        ...
        for (StoredCase sc : cases) {
            ...
            result.add(new CbrRecordSummary(
                    sc.caseId(), sc.subjectId(), sc.caseType(),
                    sc.cbrRecord().producerAgentId(), sc.cbrRecord().trustScore(), sc.storedAt()));
    ...
    public List<String> findCaseIds(String tenantId, MemoryDomain domain,
        ...
        for (StoredCase stored : cases) {
            ...
            if (stored.supersededAt() != null) continue;
            if (!filters.isEmpty() && !matchesFilters(stored.cbrRecord(), filters, schema)) continue;
    ...
    @SuppressWarnings("unchecked")
    private boolean matchesFilters(CbrRecord storedCase, Map<String, CbrFilter> filters,
        ...
        for (var entry : filters.entrySet()) {
            ...
            FeatureField field = CbrRecordValidator.findField(schema, fieldName);
    ...
    private record StoredCase(
            String id, CbrRecord cbrRecord, String caseType, String subjectId, MemoryDomain domain,
        ...
        StoredCase withSupersession(Instant supersededAt, String supersedingCaseId, String supersessionReason) {
            return new StoredCase(id, cbrRecord, caseType, subjectId, domain, tenantId, caseId, storedAt, lastOutcomeAt,
