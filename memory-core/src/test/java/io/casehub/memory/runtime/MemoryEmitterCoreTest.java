...
class MemoryEmitterCoreTest {

    private static final MemoryDomain DOMAIN = new MemoryDomain("test");
    private static final MemoryInput SAMPLE = new MemoryInput(
            "entity-1", DOMAIN, "tenant-1", null, "sample text", Map.of(), null, null, null, null);

    @Test void emit_delegates_to_store() {
        var store = new RecordingStore();
        var emitter = new MemoryEmitterCore(store);
        emitter.emit(SAMPLE);
        assertThat(store.storedInputs).containsExactly(SAMPLE);
    }

    @Test void emitAll_delegates_to_storeAll() {
        var input2 = new MemoryInput("entity-2", DOMAIN, "tenant-1", null, "text 2", Map.of(), null, null, null, null);
        var store = new RecordingStore();
        var emitter = new MemoryEmitterCore(store);
    ...
    @Test void emitAll_empty_list_does_not_call_store() {
        var store = new RecordingStore();
        var emitter = new MemoryEmitterCore(store);
    ...
    @Test void emitAll_logs_partial_failures_without_throwing() {
    ...
    private static class RecordingStore implements CaseMemoryStore {
        ...
        @Override public String store(MemoryInput input) { storedInputs.add(input); return "mem-" + storedInputs.size(); }
        ...
        @Override public StoreAllResult storeAll(List<MemoryInput> inputs) {
            ...
            return new StoreAllResult(inputs.stream().map(i -> "mem-" + i.entityId()).toList(), List.of());
        }
    }

    private static class ThrowingStore implements CaseMemoryStore {
        private final RuntimeException toThrow;
        ThrowingStore(RuntimeException toThrow) { this.toThrow = toThrow; }
        @Override public String store(MemoryInput input) { throw toThrow; }
        @Override public List<Memory> query(MemoryQuery query) { return List.of(); }
        @Override public int erase(EraseRequest request) { return 0; }
        @Override public StoreAllResult storeAll(List<MemoryInput> inputs) { throw toThrow; }
    }
