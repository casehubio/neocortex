...
class TrendEnrichmentCbrRecordStoreTest {

    private static final MemoryDomain CBR = new MemoryDomain("cbr");

    private static final CbrRecordSchema SCHEMA = CbrRecordSchema.of("clinical",
    ...
    void registerSchema_expandsSchema() {
        var capturedSchema = new AtomicReference<CbrRecordSchema>();
        var delegate = stubDelegate(capturedSchema, null, null);
        var decorator = new TrendEnrichmentCbrRecordStore(delegate);
    ...
    void store_enrichesCaseFeatures() {
        var capturedCase = new AtomicReference<CbrRecord>();
        ...
        var cbrRecord = new CbrFeatureRecord("prob", "sol", null, null,
                                           Map.of("drug", string("aspirin"), "vitals", FeatureValue.structList(obs)), null, null);
        decorator.store(cbrRecord, "clinical", "e1", CBR, "t1", null, io.casehub.platform.api.path.Path.root());
    ...
    void retrieveSimilar_enrichesQueryFeatures() {
        ...
        decorator.retrieveSimilar(query, CbrFeatureRecord.class);
    ...
    void noTrendSpec_passesThrough() {
        var noTrendSchema = CbrRecordSchema.of("plain",
                                               FeatureField.categorical("drug"));
        var capturedCase = new AtomicReference<CbrRecord>();
        var delegate = stubDelegate(null, capturedCase, null);
        var decorator = new TrendEnrichmentCbrRecordStore(delegate);
        ...
        decorator.store(cbrRecord, "plain", "e1", CBR, "t1", null, io.casehub.platform.api.path.Path.root());
    ...
    void unregisteredCaseType_passesThrough() {
        ...
        var cbrRecord = new CbrFeatureRecord("prob", "sol", null, null,
                                           Map.of("drug", string("aspirin")), null, null);
        decorator.store(cbrRecord, "unknown", "e1", CBR, "t1", null, io.casehub.platform.api.path.Path.root());

        assertThat(capturedCase.get()).isSameAs(cbrRecord);
    }

    @SuppressWarnings("unchecked")
    private CbrRecordStore stubDelegate(AtomicReference<CbrRecordSchema> capturedSchema,
                                        AtomicReference<CbrRecord> capturedCase,
        ...
        return new CbrRecordStore() {
            ...
            @Override
            public String store(CbrRecord c, String t, String e, MemoryDomain d, String tid, String cid, io.casehub.platform.api.path.Path scope) {
                if (capturedCase != null) capturedCase.set(c);
                return "id";
            }

            @Override
            public <C extends CbrRecord> List<CbrMatch<C>> retrieveSimilar(CbrQuery q, Class<C> cl) {
