...
class TrustWeightedCbrRecordStoreTest {
    ...
    @Test void nullTrustScore_passesThrough() {
        ...
        var delegate = stubDelegate(List.of(new CbrMatch<>(c, "c1", "test-type", 0.9)));
        var decorator = new TrustWeightedCbrRecordStore(delegate, fn, (AgentTrustProvider) null);
    ...
    @Test void mixedNullAndNonNull_onlyWeightsNonNull() {
        ...
        var delegate = stubDelegate(List.of(
                new CbrMatch<>(trusted, "c1", "test-type", 0.8),
                new CbrMatch<>(unknown, "c2", "test-type", 0.8)));
        var decorator = new TrustWeightedCbrRecordStore(delegate, fn, (AgentTrustProvider) null);
        var results = decorator.retrieveSimilar(testQuery(), CbrFeatureRecord.class);
    ...
    @Test void resortsAfterWeighting() {
        ...
        var delegate = stubDelegate(List.of(
                new CbrMatch<>(lowTrust, "c1", "test-type", 0.9),
                new CbrMatch<>(highTrust, "c2", "test-type", 0.85)));
        var decorator = new TrustWeightedCbrRecordStore(delegate, fn, (AgentTrustProvider) null);
        var results = decorator.retrieveSimilar(testQuery(), CbrFeatureRecord.class);
    ...
    @Test void trajectoryCache_singleCallPerAgent() {
        ...
        var delegate = stubDelegate(List.of(
                new CbrMatch<>(c1, "c1", "test-type", 0.9),
                new CbrMatch<>(c2, "c2", "test-type", 0.8)));
        var decorator = new TrustWeightedCbrRecordStore(delegate, fn, provider);
        decorator.retrieveSimilar(testQuery(), CbrFeatureRecord.class);
    ...
    @Test void trustTrajectory_storedOnCbrMatch() {
        var c = testCase("p", 0.8, "agent-1");
    ...
    private CbrFeatureRecord testCase(String problem, Double trustScore, String agentId) {
        return new CbrFeatureRecord(problem, "sol", null, null, Map.of(), trustScore, agentId);
    }
