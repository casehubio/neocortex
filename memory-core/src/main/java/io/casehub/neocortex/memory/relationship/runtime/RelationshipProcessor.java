...
public class RelationshipProcessor {
    ...
    private final Consumer<RelationshipRecorded> recorded;

    public RelationshipProcessor(CaseMemoryStore store, Consumer<RelationshipRecorded> recorded) {
        this.store = store;
    ...
    public void onExperienceRecorded(ExperienceRecorded event) {
        ...
        try {
            ...
            recorded.accept(new RelationshipRecorded(relEvent, memoryId));
        } catch (SecurityException e) {
            throw e;
        } catch (Exception e) {
            LOG.log(Level.WARNING, "Relationship memory store failed for agent=" +
                exp.agentId() + " other=" + otherAgent + " tenant=" + exp.tenantId(), e);
        }
    }
