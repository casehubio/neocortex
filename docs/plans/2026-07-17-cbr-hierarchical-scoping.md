...
**Architecture:** Scope is orthogonal to domain — a new `Path scope` dimension on storage and retrieval. Store-level filtering restricts visibility to cases at the query scope or its ancestors (no upward leakage). A `ScopeDecayCbrRecordStore` decorator applies configurable score decay by scope depth distance, symmetric with the existing `TemporalDecay` pattern.
...
### Task 2: SPI surface changes — CbrQuery, CbrMatch, CbrRecordStore, ReactiveCbrRecordStore
...
- Produces: Updated `CbrQuery` (adds `Path scope`, `ScopeDecay scopeDecay`), updated `CbrMatch` (adds `Path scope`), updated `CbrRecordStore.store()` and `ReactiveCbrRecordStore.store()` (adds `Path scope` parameter)

- [ ] **Step 1: Update CbrRecordStore.store() signature**
...
Use `ide_edit_member` to replace the `store` method declaration on `CbrRecordStore`.

- [ ] **Step 2: Update ReactiveCbrRecordStore.store() signature**
...
Uni<String> store(CbrRecord cbrCase, String caseType, String entityId, MemoryDomain domain,
                  String tenantId, String caseId, Path scope);
```

Use `ide_edit_member` on `ReactiveCbrRecordStore`.
...
- [ ] **Step 4: Update CbrMatch — add scope field**
...
public CbrMatch<C> withScore(double newScore) {
    return new CbrMatch<>(cbrCase, caseId, newScore, reranked, featureSimilarities, storedAt, scope);
}
public CbrMatch<C> withReranked() {
    return new CbrMatch<>(cbrCase, caseId, score, true, featureSimilarities, storedAt, scope);
}
```
...
git -C /Users/mdproctor/claude/casehub/neocortex commit -m "feat(#153): SPI scope changes — CbrQuery, CbrMatch, store() gain Path scope"
```
...
**Interfaces:**
- Consumes: Updated `CbrRecordStore`, `CbrQuery`, `CbrMatch` from Task 2
- Produces: Scope contract tests (6 new tests), scope-aware `InMemoryCbrRecordStore`

- [ ] **Step 1: Fix all existing store() calls in CbrRecordStoreContractTest**
...
Add these tests to `CbrRecordStoreContractTest`:
...
- [ ] **Step 3: Update InMemoryCbrRecordStore**

Add `Path scope` to `StoredCase` record. Update `store()` to accept and record scope. Update `retrieveSimilar()` to filter by scope visibility. Update `CbrMatch` construction to include scope.

Key changes to `store()`:
```java
@Override
public String store(CbrRecord cbrCase, String caseType, String entityId, MemoryDomain domain,
                    String tenantId, String caseId, Path scope) {
    // ... existing validation ...
...
private record StoredCase(
    String id, CbrRecord cbrCase, String caseType, String entityId, MemoryDomain domain,
...
- Modify: `memory/src/main/java/io/casehub/neocortex/memory/cbr/runtime/TemporalDecayCbrRecordStore.java`
- Modify: `memory/src/main/java/io/casehub/neocortex/memory/cbr/runtime/ReactiveTemporalDecayCbrRecordStore.java`
- Modify: `memory/src/main/java/io/casehub/neocortex/memory/cbr/runtime/OutcomeWeightingCbrRecordStore.java`
- Modify: `memory/src/main/java/io/casehub/neocortex/memory/cbr/runtime/ReactiveOutcomeWeightingCbrRecordStore.java`
- Modify: `memory/src/main/java/io/casehub/neocortex/memory/cbr/runtime/TrendEnrichmentCbrRecordStore.java`
- Modify: `memory/src/main/java/io/casehub/neocortex/memory/cbr/runtime/ReactiveTrendEnrichmentCbrRecordStore.java`
- Create: `memory/src/main/java/io/casehub/neocortex/memory/cbr/runtime/ScopeDecayCbrRecordStore.java`
- Create: `memory/src/main/java/io/casehub/neocortex/memory/cbr/runtime/ReactiveScopeDecayCbrRecordStore.java`
- Create: `memory/src/test/java/io/casehub/neocortex/memory/cbr/runtime/ScopeDecayCbrRecordStoreTest.java`
- Modify: `memory/src/test/java/io/casehub/neocortex/memory/cbr/runtime/TemporalDecayCbrRecordStoreTest.java`
- Modify: `memory/src/test/java/io/casehub/neocortex/memory/cbr/runtime/OutcomeWeightingCbrRecordStoreTest.java`
- Modify: `memory/src/test/java/io/casehub/neocortex/memory/cbr/runtime/ReactiveOutcomeWeightingCbrRecordStoreTest.java`
- Modify: `memory/src/test/java/io/casehub/neocortex/memory/cbr/runtime/TrendEnrichmentCbrRecordStoreTest.java`

**Interfaces:**
- Consumes: Updated `CbrRecordStore`, `ScopeDecay`, `CbrMatch` from Tasks 1-2
- Produces: `ScopeDecayCbrRecordStore` @Decorator @Priority (85), all decorators forwarding scope in store ()
...
```java
@Override public String store(CbrRecord c, String ct, String e, MemoryDomain d, String t, String ci, Path scope) {
...
- `NoOpCbrRecordStore` — also update return `""`
- `BlockingToReactiveCbrBridge` — update lambda: `delegate.store(cbrCase, caseType, entityId, domain, tenantId, caseId, scope)`
- `TemporalDecayCbrRecordStore` + `ReactiveTemporalDecayCbrRecordStore`
- `OutcomeWeightingCbrRecordStore` + `ReactiveOutcomeWeightingCbrRecordStore`
- `TrendEnrichmentCbrRecordStore` + `ReactiveTrendEnrichmentCbrRecordStore`
...
- `TemporalDecayCbrRecordStoreTest`
- `OutcomeWeightingCbrRecordStoreTest`
- `ReactiveOutcomeWeightingCbrRecordStoreTest`
- `TrendEnrichmentCbrRecordStoreTest`

- [ ] **Step 3: Write ScopeDecay decorator tests**

Create `ScopeDecayCbrRecordStoreTest.java`:
...
class ScopeDecayCbrRecordStoreTest {
    ...
    private ScopeDecayCbrRecordStore decorator(List<CbrMatch<CbrFeatureRecord>> results) {
        CbrRecordStore stub = new StubStore(results);
        return new ScopeDecayCbrRecordStore(stub);
    }

    private CbrMatch<CbrFeatureRecord> scored(double score, Path scope) {
        var c = new CbrFeatureRecord("p", "s", null, null, Map.of());
        return new CbrMatch<>(c, "id", score, false, Map.of(), Instant.now(), scope);
    }
...
- [ ] **Step 4: Implement ScopeDecayCbrRecordStore**
...
@Decorator
@Priority(85)
public class ScopeDecayCbrRecordStore implements CbrRecordStore {

    private final CbrRecordStore delegate;

    @Inject
    public ScopeDecayCbrRecordStore(@Delegate @Any CbrRecordStore delegate) {
...
- [ ] **Step 5: Implement ReactiveScopeDecayCbrRecordStore**

Create `memory/src/main/java/io/casehub/neocortex/memory/cbr/runtime/ReactiveScopeDecayCbrRecordStore.java` — same logic as blocking variant but with `Uni<List<CbrMatch<C>>>` return and Mutiny operators. Follow the `ReactiveTemporalDecayCbrRecordStore` pattern exactly.
...
**Files:**
- Modify: `memory-cbr-crossencoder/src/main/java/io/casehub/neocortex/memory/cbr/crossencoder/RerankingCbrRecordStore.java`
- Modify: `memory-cbr-crossencoder/src/main/java/io/casehub/neocortex/memory/cbr/crossencoder/ReactiveRerankingCbrRecordStore.java`
- Modify: `memory-cbr-tracking/src/main/java/io/casehub/neocortex/memory/cbr/tracking/TrackingCbrRecordStore.java`
- Modify: `memory-cbr-tracking/src/main/java/io/casehub/neocortex/memory/cbr/tracking/ReactiveTrackingCbrRecordStore.java`
- Modify: corresponding test files for any `CbrQuery.of()` or `store()` call site updates

**Interfaces:**
- Consumes: Updated `CbrRecordStore` from Task 2
- Produces: All cross-encoder and tracking decorators compile and pass tests

- [ ] **Step 1: Update cross-encoder decorators**

Update `store()` pass-through in `RerankingCbrRecordStore` and `ReactiveRerankingCbrRecordStore` to include `Path scope`. Add import.

- [ ] **Step 2: Update tracking decorators**

Update `store()` pass-through in `TrackingCbrRecordStore` and `ReactiveTrackingCbrRecordStore` to include `Path scope`. Add import.
...
**Files:**
- Modify: `memory-cbr-jpa/src/main/java/io/casehub/neocortex/memory/cbr/jpa/CbrRecordEntity.java`
...
**Interfaces:**
- Consumes: Updated `CbrRecordStore`, `CbrQuery`, `CbrMatch` from Task 2, `Path` from platform-api
...
- [ ] **Step 2: Add scope to CbrRecordEntity**
...
Update `CbrMatch` construction to include `Path.parse(entity.scope)` (or `Path.root()` if scope is `""`).
...
**Interfaces:**
- Consumes: Updated `CbrRecordStore`, `CbrQuery`, `CbrMatch`, `Path` from Tasks 1-2
...
- [ ] **Step 4: Update QdrantCbrRecordStore.store() — add scope to payload**
...
- [ ] **Step 5: Update QdrantCbrRecordStore.retrieveSimilar() — extract scope from payload**

When constructing `CbrMatch` from Qdrant results, extract scope:
```java
...
Include `scope` in `CbrMatch` construction.
