...
That framing led to the `CbrCase` type hierarchy — an open interface (not sealed) with `ResolutionGuide`, `FeatureVectorCbrCase`, and eventually `ResolvedCase`. The base carries `problem()`, `solution()`, `outcome()`, `confidence()` — universal across all CBR paradigms. Subtypes add structured features or plan traces. The open hierarchy means new paradigms (Structural, Graph) can be added without modifying the base.
