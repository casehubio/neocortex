...
/**
 * Configuration for loading an ONNX model.
 *
 * @param modelPath          path to the ONNX model file
 * @param tokenizerPath      path to the HuggingFace tokenizer.json (null for tensor-only models)
 * @param maxSequenceLength  maximum token sequence length (truncation boundary, ignored for tensor-only)
...
public record ModelConfig(
    ...
    public ModelConfig {
        ...
        if (intraOpThreads < 0) {throw new IllegalArgumentException("intraOpThreads must be non-negative");}
        if (interOpThreads < 0) {throw new IllegalArgumentException("interOpThreads must be non-negative");}
    ...
    /**
     * Tensor-only: no tokenizer, no sequence length.
     */
    public ModelConfig(Path modelPath) {
        this(modelPath, null, 1, 0, 0, null);
    }
}
