...
cognitive-api/      — zero deps: Confidence record, ConfidenceOrigin enum, TemporalMark sealed hierarchy (WallClock, Relative, Ordinal), AffectType enum (INHERENT, ANTICIPATORY), ModulationProfile<T> (accessor record for generic field extraction), ModulationFactor<T> (@FunctionalInterface — composable scoring multiplier), RetrievalModulator (static utility: applies factors via multiplication, sorts by composite score). Cross-cutting cognitive types shared by MindMap, Memory, and CBR subsystems.
cognitive-index/    — cross-store cognitive query tier: TemporalIndex (chronological aggregation across MindMap + Memory + CBR), TemporalEntry, TemporalSource (sealed), TemporalQuery, TemporalRanker (@FunctionalInterface), AffectTrajectoryAnalyzer (slope/volatility/trend from domain="affect" memories), AffectTrajectory, TrendDirection. Stateless derived view — re-queries stores on demand, no persistence. @ApplicationScoped CDI bean with Instance<T> graceful degradation. TemporalFocus (static utility: ranked attention list with proximity/recency/trajectory scoring), AttentionItem (scored entry + reason), TemporalFocusConfig (tunable thresholds). CognitiveProfile (@ApplicationScoped — cross-store entity resolution: resolves MindMap node + edges + memories across 6 domains + affect trajectory + NodeRef following into unified EntityKnowledge record; Instance<T> graceful degradation; configurable via CognitiveProfileQuery with byId/byName factories, domain selection, edge/memory limit), CognitiveProfileQuery (query record), EntityKnowledge (result record). ModulationContext (convenience record: MoodState + PersonalityWeights + Instant), ModulationProfiles (pre-built: MEMORY, NODE), ModulationFactors (factories: recencyDecay, confidenceWeight, moodCongruence, domainWeight). PerspectivalMerge (pure static utility: merge shared MindMapNode + private overlay → single perspectival node; overlay PAD/confidence/properties win when non-null), PerspectivalResolver (@ApplicationScoped — resolves perspectival overlays: finds agent's overlay nodes via trait-based search + agentId property filter within same tenant, merges with shared nodes; Instance<MindMapStore> graceful degradation). CognitiveDefaults (per-agent cognitive config record from YAML — personality, mood baseline, curiosity, temporal focus, cbrStrategy, socialCognition, graphStructure, extractionBias, vocabulary, services map for @Named SPI selection, traitRules + derivedEdgeRules nullable for declarative rule overrides), CbrStrategyDefaults (minSimilarity, temporalDecayDays, RetrievalMode — CBR retrieval parameter defaults derived from disposition), SocialCognitionDefaults (trustFormationRate, ConflictInterpretation — trust and conflict behaviour derived from disposition), GraphStructureDefaults (InferenceStyle CONNECTIVE/CATEGORICAL/BALANCED, connectiveBias — knowledge graph organisation style derived from disposition profile), ExtractionBiasDefaults (relationshipBias, affectSensitivity — LLM extraction emphasis derived from disposition profile), CognitiveDefaultsRegistry (classpath scan for cognitive-profiles/*.yaml, runtime lookup by agentId via forAgent/forAgentOrDefaults), PersonalityWeightsDeserializer (Jackson MemoryDomain key wrapping for YAML parsing). RuleConditionDeserializer (Jackson YAML → RuleCondition tree), DeclarativeTraitRuleDeserializer, DeclarativeDerivedEdgeRuleDeserializer (Jackson YAML → declarative rule types). DeclarativeRuleRegistry (@ApplicationScoped — loads global rules from rules/*.yaml + per-agent overrides from cognitive profiles, name-based override merge — local rule with same name suppresses global). DescriptorView (agentId, DispositionAxes, dispositionProfile WeightedTerms, goals — read-only eidos identity view, zero eidos compile dependency), CognitiveDerivationEngine (pure static utility: DescriptorView → CognitiveDefaults — 8 derivation pathways: Jungian disposition profile → PersonalityWeights weighted average, disposition axes → MoodBaseline PAD mapping, disposition+goals → CuriosityConfig categoryWeights, goals → TemporalFocusConfig subgraphProximityWeights, ruleFollowing+riskAppetite → CbrStrategyDefaults, socialOrient+conflictMode → SocialCognitionDefaults, disposition profile → GraphStructureDefaults inferenceStyle, disposition profile → ExtractionBiasDefaults; deriveAndMerge overlays explicit overrides on derived base).
...
mindmap-api/        — MindMapStore SPI, MindMapNode + MindMapEdge interfaces, MindMapSubgraph, MindMapVocabulary (EdgeTypeDefinition with aliases + decay half-life), MindMapQuery, MindMapCapability enum (TRAVERSAL, MERGE, VOCABULARY, ALIAS, SUBGRAPH, SEARCH, SUPERSESSION, ERASE_NODE, ERASE_SUBGRAPH, ERASE_ENTITY, CROSS_TENANT_ERASE, GRAPH_ANALYSIS), ConfidenceOrigin (STATED/INFERRED/SPECULATED with initial confidence), ValidationTier (REGISTERED/UNVALIDATED), SubgraphType (PERSON/PROJECT/RESEARCH_AREA/ORGANISATION/CONCEPT/GENERAL), NodeInput, EdgeInput, NodeUpdate (additive trait/ref/property mutations), SubgraphInput, NodeRef (scheme/id/qualifier for external references), MergeResult + MergeConflict, SupersessionStatus (with reinstatement tracking, NOT_SUPERSEDED constant), MindMapCapabilityException, VocabularyConflictException, DerivedEdgeRule (forward-chaining rule SPI with provenance property constants), RecurrenceRule record (Frequency enum, parse/toString for RFC 5545 RRULE subset: FREQ/INTERVAL/COUNT/UNTIL/BYDAY). Nodes carry PAD emotional dimensions (pleasure/arousal/dominance), temporal validity (validFrom/validUntil), traits, refs, arbitrary properties. OverlayRef (convention for perspectival affect overlays — NodeRef scheme="overlay", trait "overlay"; of(sharedNodeId) factory, sharedNodeId(MindMapNode) extractor), CuriosityConfig (11-field tuning record with defaults() factory — proximity, staleness, trajectory thresholds, categoryWeights map for per-SignalCategory score multipliers + categoryWeight(String) accessor + withCategoryWeights()). Zero deps.
mindmap/            — CDI wiring: NoOpMindMapStore (@DefaultBean @ApplicationScoped), VocabularyNormalizationDecorator (edge type alias resolution), ConfidenceDecayDecorator (exponential half-life decay on node confirmedAt / edge updatedAt), DerivedEdgeDecorator (@Decorator @Priority(80) — forward-chaining rules on addEdge, truth maintenance on removeEdge, cycle prevention max depth 3, provenance tracking via edge properties, Instance<DeclarativeRuleRegistry> for combined programmatic + declarative rule discovery), AffectTrajectoryDecorator (@Decorator @Priority(65) — intercepts updateNode PAD changes, stores domain="affect" memories via CaseMemoryStore, fires AffectRecorded CDI event; Instance<CaseMemoryStore> for graceful degradation), MindMapAnalyzer (static utility — pure Java graph analysis: orphanNodes, degreeCentrality, subgraphDensity, unvalidatedEdgeRatio, contradictions, lowConfidenceCluster, staleNodes, betweennessCentrality; requires GRAPH_ANALYSIS capability)
...
| Schema Generator | `casehub-neocortex-schema-generator` |
...
| Root Java package (mindmap-intelligence) | `io.casehub.neocortex.mindmap.intelligence` |
| Root Java package (schema-generator) | `io.casehub.neocortex.schema` |
