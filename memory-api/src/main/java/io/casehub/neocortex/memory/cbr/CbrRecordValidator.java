...
public final class CbrRecordValidator {

    private CbrRecordValidator() {}

    public static void validateStoreFeatures(Map<String, FeatureValue> features, CbrRecordSchema schema) {
    ...
    public static void validateQueryFeatures(Map<String, FeatureValue> features, CbrRecordSchema schema) {
        for (var entry : features.entrySet()) {
            FeatureField field = findField(schema, entry.getKey());
            if (field == null) {continue;}
            FeatureValue value = entry.getValue();
    ...
    public static void validateFilters(Map<String, CbrFilter> filters, CbrRecordSchema schema) {
    ...
    private static void requireCategorical(String name, FeatureField field) {
        ...
        if (!(field instanceof FeatureField.CategoricalList) && !(field instanceof FeatureField.Categorical))
            throw new IllegalArgumentException(
                "Contains/ContainsAny filter on '" + name
    ...
    private static void requireCategoricalList(String name, FeatureField field) {
        ...
        if (!(field instanceof FeatureField.CategoricalList))
            throw new IllegalArgumentException(
                "ContainsAll/NotContains/NotContainsAny filter on '" + name
    ...
    public static FeatureField findField(CbrRecordSchema schema, String name) {
