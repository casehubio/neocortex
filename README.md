...
## memory-* — case-based reasoning for case steps

### CbrCaseMemoryStore — structured similarity search

Every CaseHub application is a natural CBR system: it faces problems (cases), has a history of how similar problems were handled, and should learn from that history rather than starting from scratch each time. `CbrCaseMemoryStore` provides structured feature-vector similarity search over past cases — categorical exact match, numeric range filtering, and optional dense vector similarity on text fields.

Three CBR paradigms are supported: **Textual** (pure NL similarity), **Feature-Vector** (structured categorical/numeric/text features), and **Plan-Based** (execution traces for CHEF-style case-based planning). See [CBR Types](docs/cbr/cbr-types.md) for a detailed explanation of each.

Applications across the platform use CBR for domain-specific decision support: AML investigation precedent, clinical adverse event history, PR review patterns, contractor coordination, IoT situation handling, and game-plan adaptation. The SPI is domain-agnostic — the same store, same query API, different schemas.

- [CBR Types — what they are and when to use them](docs/cbr/cbr-types.md)
- [CBR Integration Guide — SPI, schemas, backends, Maven](docs/cbr/README.md)
...
| `memory-api/` | `casehub-neocortex-memory-api` | Pure Java | `CbrCaseMemoryStore`, `CbrCase` hierarchy, `CbrQuery`, `CbrFeatureSchema` |
| `memory/` | `casehub-neocortex-memory` | Quarkus | `NoOpCbrCaseMemoryStore` default + `BlockingToReactiveCbrBridge` |
| `memory-cbr-inmem/` | `casehub-neocortex-memory-cbr-inmem` | Pure Java | In-memory stub — categorical exact match, no persistence |
| `memory-qdrant/` | `casehub-neocortex-memory-qdrant` | Quarkus | Qdrant backend — payload filters + dense vector on `problem()` |
| `memory-testing/` | `casehub-neocortex-memory-testing` | Pure Java | `CbrCaseMemoryStoreContractTest` — 26-test contract suite |
